...
BACKEND_HEALTH_URL="${VOXLOG_BACKEND_HEALTH_URL:-http://127.0.0.1:7892/health}"
TS_HEALTH_URL="${VOXLOG_TS_HEALTH_URL:-http://127.0.0.1:7891/health}"
GATEWAY_HEALTH_URL="${VOXLOG_GATEWAY_HEALTH_URL:-http://127.0.0.1:7893/health}"
...
echo "VoxLog alpha doctor"
check_file "Desktop app" "$APP_TARGET"
check_file "Env file" "$ENV_FILE"
check_file "Desktop settings" "$SETTINGS_FILE"
check_file "Backend launch agent" "$LAUNCH_AGENTS_DIR/com.voxlog.backend.plist"
check_file "API TS launch agent" "$LAUNCH_AGENTS_DIR/com.voxlog.api-ts.plist"
check_file "Gateway launch agent" "$LAUNCH_AGENTS_DIR/com.voxlog.gateway.plist"
